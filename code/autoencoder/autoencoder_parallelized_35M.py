...
def train_autoencoder(model, embedding_path, epochs=1, batch_size=4096, lr=1e-3):
    ...
    torch.save(model.state_dict(), "autoencoder_optimized_35M_our_model.pth")
    print("\nModel saved as autoencoder_optimized_35M_our_model.pth",flush=True)

if __name__ == "__main__":
    embeddings_training = "/leonardo_scratch/large/userexternal/pjardiya/jobs_embeddings/all_combined/35M_our_model_embedding_all_all.npy"
    print("Initializing model...", flush=True)
    model = ESMEmbeddingAutoencoder(input_dim=480, latent_dim=64)
