...
INPUT_DIR="/leonardo_scratch/large/userexternal/pjardiya/embeddings/embeddings_35M_our_model/"
OUTPUT_DIR="/leonardo_scratch/large/userexternal/pjardiya/jobs_embeddings/jobs_embedding_35M_our_model"
mkdir -p "$OUTPUT_DIR"

python3 per_job_compress_train_embeddings.py \
  --job-id "$SLURM_ARRAY_TASK_ID" \
  --input-dir "$INPUT_DIR" \
  --output "$OUTPUT_DIR/job${SLURM_ARRAY_TASK_ID}_embedding_all_35M_our_model.npy" \
  --workers 32
